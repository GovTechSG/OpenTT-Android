...
class GattServer constructor(val context: Context, serviceUUIDString: String) {
    ...
    private val gattServerCallback = object : BluetoothGattServerCallback() {
        ...
        override fun onCharacteristicWriteRequest(
            ...
            if (device == null) {
                ...
                CentralLog.e(loggerTAG, "Write stopped - no device")
                DBLogger.e(
                    DBLogger.LogType.BLUETRACE,
                    loggerTAG,
                    "Write stopped - no device",
                    null
                )
            }
        ...
        fun saveDataReceived(device: BluetoothDevice) {
            ...
            CentralLog.i(TAG, "save bt data received from: ${device.address}" )
            ...
            charUUID?.let {
                data?.let {
                    try {
                        ...
                        val loggerTAG = "${javaClass.simpleName} -> ${object{}.javaClass.enclosingMethod?.name}"
                        CentralLog.e(loggerTAG, "Failed to process write payload - ${e.message}")
                        DBLogger.e(
                            DBLogger.LogType.BLUETRACE,
                            loggerTAG,
                            "Failed to process write payload - ${e.message}",
    ...
    fun stop() {
        try {
            ...
            val loggerTAG = "${javaClass.simpleName} -> ${object{}.javaClass.enclosingMethod?.name}"
            CentralLog.e(loggerTAG, "GATT server can't be closed elegantly ${e.localizedMessage}")
            DBLogger.e(
                DBLogger.LogType.BLUETRACE,
                loggerTAG,
                "GATT server can't be closed elegantly ${e.localizedMessage}",
                DBLogger.getStackTraceInJSONArrayString(e as Exception)
            )
        }
    }
