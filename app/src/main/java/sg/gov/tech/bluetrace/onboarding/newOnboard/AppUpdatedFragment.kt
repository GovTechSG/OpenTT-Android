...
class AppUpdatedFragment : Fragment() {
...
//        var inflatedView =  inflater.inflate(R.layout.fragment_app_updated, container, false)
    ...
    private fun inflateContent(root: ViewGroup): View {
        return LayoutInflater.from(requireContext())
            .inflate(R.layout.fragment_app_updated, root, true)
    }
    ...
    fun setupButtons(view: View) {
        view.findViewById<View>(R.id.btn_start).setOnClickListener {
            val intent = Intent(context, MainOnboardingActivity::class.java)
            if (FirebaseAuth.getInstance().currentUser == null) {
                //already onboard previously, but brute force to skip our authentication.
                intent.putExtra("page", Fragments.VERIFY_NUMBER.id)
            } else {
                //already onboard previously, need them to provide NRIC information.
                intent.putExtra("page", Fragments.SELECT_ID_DOCUMENT.id)
            }
