...
class WpViewModelTest : KoinTest {
    private val wpVm by inject<WpViewModel>()
    var registrationData = MutableLiveData<ApiResponseModel<out Any>>()
    lateinit var registerUserData: RegisterUserData
    ...
    fun `test post value Name`() {
        wpVm.postValue(WpViewModel.NAME, "Sam") { assert(it) }
    }

    @Test
    fun `test post value DOA`() {
        wpVm.postValue(WpViewModel.APPLICATION_DATE, 1577808000000) { assert(it) }
    }
    ...
    fun `test is all field valid`() {
        wpVm.postValue(WpViewModel.NAME, "Sam") { assert(it) }
        wpVm.postValue(WpViewModel.APPLICATION_DATE, 895593600000) { assert(it) }
        wpVm.postValue(WpViewModel.SERIAL_NUMBER, "K12939393939") { assert(it) }
        wpVm.postValue(WpViewModel.FIN, "S3188211G") { assert(!it) }
    }

    @Test
    fun `test is all field invalid`() {
        wpVm.postValue(WpViewModel.NAME, "") { assert(!it) }
        wpVm.postValue(WpViewModel.APPLICATION_DATE, 1643377068000) { assert(!it) }
        wpVm.postValue(WpViewModel.SERIAL_NUMBER, "") { assert(!it) }
        wpVm.postValue(WpViewModel.FIN, "S38211G") { assert(!it) }
    }
