...
class DpViewModelTest : KoinTest {


    private val dpVm by inject<DpViewModel>()
    ...
    fun `test post value Name`() {
        dpVm.postValue(DpViewModel.NAME, "Sam") { assert(it) }
    }

    @Test
    fun `test post value card serial number`() {
        dpVm.postValue(DpViewModel.SERIAL_NUMBER, "K7712300") { assert(it) }
    }
    ...
    fun `test is form incomplete`() {
        ...
        dpVm.isFormComplete(hashValues) { assert(!it) }
    }
    ...
    fun `test is all field valid`() {
        dpVm.postValue(DpViewModel.NAME, "Sam") { assert(it) }
        dpVm.postValue(DpViewModel.SERIAL_NUMBER, "K12939393939") { assert(it) }
        dpVm.postValue(DpViewModel.FIN, "S3188211G") { assert(!it) }
    }

    @Test
    fun `test is all field invalid`() {
        dpVm.postValue(DpViewModel.NAME, "") { assert(!it) }
        dpVm.postValue(DpViewModel.SERIAL_NUMBER, "") { assert(!it) }
        dpVm.postValue(DpViewModel.FIN, "G5996561") { assert(!it) }
    }

    @Test
    fun `test register user`() = runBlockingTest {
        val paramString = Gson().toJson(registerUserData, RegisterUserData::class.java)
        ...
        dpVm.registerUser(registerUserData)
    }
    ...
    val registerUserData = UpdateUserInfoWithPolicyVersion(
        IdentityType.FIN_WP,
        "S3188211G",
        "09-01-2020",
//                    dob,
